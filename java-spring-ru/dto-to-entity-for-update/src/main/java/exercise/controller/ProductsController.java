...
public class ProductsController {
    ...
    public ProductDTO update(@PathVariable Long id, @RequestBody ProductUpdateDTO productUpdateData) {
        var product = productRepository.findById(id)
                .orElseThrow(() -> new ResourceNotFoundException("Product with id " + id + " not found"));
        toEntity(productUpdateData, product);
        productRepository.save(product);
        return toDTO(product);
    }

    private Product toEntity(ProductUpdateDTO productUpdateData, Product product) {
        product.setTitle(productUpdateData.getTitle());
